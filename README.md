...
2. Запустите [Overleaf](https://ru.overleaf.com/project)
3. Импортируйте новый проект из `.zip` файла
4. Настройте [Overleaf на XeLaTeX](https://docs.overleaf.com/getting-started/recompiling-your-project/selecting-a-tex-live-version-and-latex-compiler)
5. Начните изменять `topic/topicX.tex` вашим текстом

### Дополнительно

- Если вы работаете с LLM - изучите `GeneratingPrompt.md`. Этот файл содержит инструкцию для работы с моим шаблоном (так же может быть изучен для понимая команд программы)
- Изучите [Структура проекта](#-структура-проекта) для получения дополнительной информации работы
...
- `marker.tex` — опциональный маркер 60% высоты текста для контроля соответствия ГОСТ (красная пунктирная линия на каждой странице). Управляется флагом в файле:
  - `\showgosttrue` — включить маркер
  - `\showgostfalse` — отключить маркер
